...
python get_pred_scorer_data.py \
    ...
    $sampled_flow_dir/for_filter_input_flow.json \
    flow

# python -m debugpy --listen 5678 --wait-for-client \
python \
    pred_filter_score.py \
    --model_name_or_path outputs/flow_filter/checkpoint-16249 \
    --test_file $sampled_flow_dir/for_filter_input_flow.json \
...
python merge_ppl_score.py \
    $sampled_flow_dir/generated_dials.json \
    $sampled_flow_dir/flow_scores.json \
    $sampled_flow_dir/generated_dials_w_flow_score.json
